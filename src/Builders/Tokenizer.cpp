...
std::shared_ptr<Character>Tokenizer::next()
{
    ...
    while (this->m_index < this->m_input.length()) 
    {
        ...
        switch(c)
        {
            ...
            case '~':
                TokenizerUtilities::IncrementIndex(m_input, m_index);
                return std::make_shared<TildeSymbol>(std::string(view.substr(start, 1)));
            case '|':
                TokenizerUtilities::IncrementIndex(m_input, m_index);
                return std::make_shared<PipeSymbol>(std::string(view.substr(start, 1)));
            case '$':          
                TokenizerUtilities::IncrementIndex(m_input, m_index);
            ...
            case '^':
                TokenizerUtilities::IncrementIndex(m_input, m_index);
                return std::make_shared<CaratSymbol>(std::string(view.substr(start, 1)));
            case '*':
                TokenizerUtilities::IncrementIndex(m_input, m_index);
                return std::make_shared<AsteriskSymbol>(std::string(view.substr(start, 1)));
            case '+':  
                TokenizerUtilities::IncrementIndex(m_input, m_index);
                return std::make_shared<PlusSymbol>(std::string(view.substr(start, 1)));
            case '&':
                TokenizerUtilities::IncrementIndex(m_input, m_index);
